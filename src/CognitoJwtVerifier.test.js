...
describe('CognitoJwtVerifier()', () => {
	...
	describe('verifyToken()', () => {
		test('throws error for missing params', async () => {
			...
			await verifier.verifyToken({}).catch(e => expect(e).toMatch('"token" parameter is required.'));
			await verifier.verifyToken({token}).catch(e => expect(e).toMatch('"aws_region" parameter is required.'));
			await verifier.verifyToken({token, aws_region}).catch(e => expect(e).toMatch('"userpool_id" parameter is required.'));
			return verifier.verifyToken({token, aws_region, userpool_id}).catch(e => expect(e).toMatch('"userpool_client_id" parameter is required.'));
		});
	});
