...
const required_params = ["token", "aws_region", "userpool_id", "userpool_client_id"];
...
module.exports = class CognitoJwtVerifier {
	...
	async verifyToken(params) {
		...
		let decoded_jwt = await this.ensureTokenIsValid({token, aws_region, userpool_id});
	...
	async ensureTokenIsValid({token, aws_region, userpool_id}) {
		...
		let jwk = await this.key_manager.getKey({key_id, aws_region, userpool_id});
